...
function validateFeatureCollection(fc) {
  ...
  const groupErrors = validateGroups(fc.features);
...
function validateGroups(features) {
  let groups = {};
  let groupErrors = [];
  features.forEach(feature => {
    if (feature.properties['tofix:group']) {
      const groupName = feature.properties['tofix:group'];
      if (!groups[groupName]) {
        groups[groupName] = [];
      }
  ...
  Object.keys(groups).forEach(groupName => {
    ...
    const hasPosition = _.some(groupFeatures, groupFeature => {
      return groupFeature.properties.hasOwnProperty('tofix:group-position');
    });
    if (hasPosition) {
      groupFeatures.forEach(groupFeature => {
        if (!groupFeature.properties.hasOwnProperty('tofix:group-position')) {
          groupErrors.push(`Member of group ${groupName} does not have group-position property set`);
        }
      });
    }
  });
